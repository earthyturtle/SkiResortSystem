...
import java.awt.*; // To reduce the size of lists, whenever I am using 3 or more imports from the same solder I am opting for a * import instead.
...
public class MtBullerResortGUI {
	MtBullerDatabase db = new MtBullerDatabase();
	...
	/*
	 * This does require some explanation. I wanted to attempt using CardLayout AND JTabbedPane to create a better user interface.
	 * The idea behind this is to turn all the add and edit menus into their own separate cards. So that I can fit all the variables on 1 screen neatly.
	 * Thus reducing clutter on the main tab menu which will only contain the main function of searching for accommodations/customers/travel packages
	 * much in the same way the tutorial has it layout. So the Cards are purely just cosmetic and do not replace the tab system in any way.
	 */
